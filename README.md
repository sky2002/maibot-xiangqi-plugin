...
| 档位 | 名称 | 搜索深度上限 | 候选策略 |
| --- | --- | --- | --- |
| 1 | 入门 | 1 | 85% 概率尝试较明显的失误 |
| 2 | 简单 | 2 | 60% 概率尝试较温和的失误 |
| 3 | 标准（新局默认） | 6 | 保留引擎靠前的候选 |
| 4 | 困难 | 10 | 保留引擎靠前的候选 |
| 5 | 挑战（升级前的搜索方式） | 不额外限制深度 | 保留引擎靠前的候选 |

五档是相对对弈难度，不是经标定的人类水平。更高档允许搜索更深，但具体局面、机器速度、时间预算和 LLM 的选择都会影响实际表现，不保证每一步都比低档好，也不保证入门档适合所有初学者。深度指引擎的搜索迭代深度，不能直接理解为完整算清这么多回合。所有档位仍受 `engine.movetime_ms` 上限约束，不会突破单核隔离或增加搜索线程。

0.3.1 起，低档不再只把引擎最好的几步交给 LLM：入门/简单在对应浅深度上评估全部合法走法，再以表中概率尝试组成较弱的候选池。在这样的回合，最优招被排除，LLM 即使挑选池里最好的一步，也不能恢复到原来的最优着法。可能出现漏防、错过得子或错过杀棋，而不只是少算几层。

候选损失以本次引擎的 `cp` 评分差计算：入门优先 100–600，简单优先 50–300；它们不是 Elo 或真人段位。当没有落在目标范围的走法时，允许更小的损失；仍没有合适候选则正常选招，不强行为凑数大送子。有非必败招时，不主动提供引擎已经判为强制输棋的招；发现杀棋时，可以错过杀棋，但另选的普通评分招之间仍有损失上限。强制应将、唯一合法着法或分数接近的局面可能正常应对，所以表中概率是**尝试降强的概率，不是实际错误率或玩家胜率**。

低档交给 LLM 的候选可能少于 `engine.candidates`，不会拿强招补足数量。随机性仅用于候选池，合法性检查和 LLM 最终拍板都保留；模型失败不会随机代走。搜索更宽可能增加低档的引擎评估开销，但仍遵守原搜索时间上限。
