...
| 2 | 简单 | 1 | 60% 概率尝试较温和的失误 |
| 3 | 标准（新局默认） | 2 | 30% 概率尝试小失误 |
| 4 | 困难 | 4 | 15% 概率尝试轻微失误 |
...
候选损失以本次引擎的 `cp` 评分差计算：入门优先 100–600，简单优先 50–300，标准优先 25–150，困难优先 10–80；它们不是 Elo 或真人段位。当没有落在目标范围的走法时，允许更小的损失；仍没有合适候选则正常选招，不强行为凑数大送子。有非必败招时，不主动提供引擎已经判为强制输棋的招；发现杀棋时，可以错过杀棋，但另选的普通评分招之间仍有损失上限。强制应将、唯一合法着法或分数接近的局面可能正常应对，所以表中概率是**尝试降强的概率，不是实际错误率或玩家胜率**。

有失误机制的档位交给 LLM 的候选可能少于 `engine.candidates`，不会拿强招补足数量。随机性仅用于候选池，合法性检查和 LLM 最终拍板都保留；模型失败不会随机代走。搜索更宽可能增加引擎评估开销，但仍遵守原搜索时间上限。
...
只有棋局发起者可以调整，思考或解析走法时需等待。难度只影响下一次引擎搜索，不改棋谱、不触发落子、不延长占桌时限；普通聊天不能修改难度。设定随棋局保存，重启后保留，不影响其他群。新开局采用配置默认值或开局指令指定值；旧存档缺少难度字段时按第 6 档继续，保持最早只有时间限制的搜索方式。已保存为第 5 档的棋局仍是挑战，升级后采用深度 6；想沿用不额外限制深度的方式可设置「下棋 难度 超人类」。纯 LLM 模式不提供引擎难度调节。
